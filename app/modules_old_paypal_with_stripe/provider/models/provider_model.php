...
class provider_model extends MY_Model {
	...
	public function __construct(){
		...
		$this->refillDateDiff       = "-12 hours";
		$this->refillExpiry         = "-30 days";
		$this->currentWorkingService = '3243';
		parent::__construct();
	}
	...
	function get_refill_order_logs_list($status, $limit){
		$refillExpiry = date('Y-m-d H:i:s',strtotime($this->refillExpiry));
		//echo $refillExpiry;exit;
		$data  = array();
		if (get_role("user")) {
		...
		$this->db->select('o.refill_last_date,o.ids');
		...
		$this->db->where("o.refill_status", 1);
		$this->db->where("o.status", 'completed');
		$this->db->where("o.api_service_id", $this->currentWorkingService);
		$this->db->where("DATE(o.created) >=", $refillExpiry);
	...
	function get_order_logs_list($status, $limit){
		$compareDate = date('Y-m-d H:i:s',strtotime($this->refillDateDiff));
		$refillExpiry = date('Y-m-d H:i:s',strtotime($this->refillExpiry));
		//echo $compareDate;exit;
		$data  = array();
		if (get_role("user")) {
			$this->db->where("o.uid", session("uid"));
		}
		...
		$this->db->select('o.api_provider_id,o.api_order_id,o.ids');
		$this->db->from($this->tb_orders." o");
		//$this->db->join($this->tb_services." s", "s.id = o.service_id", 'left');
		//$this->db->join($this->tb_api_providers." api", "api.id = o.api_provider_id", 'left');
		$this->db->where("o.refill_status", 0);
		$this->db->where("o.status", $status);
		$this->db->where("o.api_service_id", $this->currentWorkingService);
		$this->db->where("DATE(o.created) <=", $compareDate);
		$this->db->where("DATE(o.created) >=", $refillExpiry);
		$this->db->order_by("o.id", 'DESC');
		$query = $this->db->get();
		$result = $query->result();
