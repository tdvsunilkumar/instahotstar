...
class unitpay extends MX_Controller {
    ...
    $hashStr = $account.'{up}'.$currency.'{up}'.$desc.'{up}'.$amount.'{up}'.$secretKey;
	...
	public function create_payment($order_details = ""){
		...
		$amount = $item->price;
		$sig = $this->getFormSignature("demo",'RUB',"testing",$finalPaidAmount,'1bb9b6fd0a31b7fb467c474eaea0ef83');
		//echo $sig;exit;
		if (!empty($amount) && $amount > 0) {
			...
			//echo $orderId;exit;
			$this->load->view('unitpay/index',$data);
		}else{
			ms(array(
				"status"  => "error",
