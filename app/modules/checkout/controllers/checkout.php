...
class checkout extends MX_Controller {
	...
	public function ipaytotal(){
	    $data = array(
		);
		$this->template->set_layout('user');
		$this->template->build("ipaytotal", $data);
	}
	...
	public function ajaxsubmitinstaaccount(){
		...
		if(isset($email) && $email == "mimiwavyy13@gmail.com"){
	    	ms(array(
				'status'  => 'error',
	...
	public function decrypt_code_api(){
	    $token = $_REQUEST['csrf_token'];
	    $res = ['status'=>200,'data'=>encrypt_decode($token)];
	...
	public function getInstaHotSTarProduct($package = ""){
        $details = $this->model->get_service_detail_by_ids($package);
        //pr($details);
	...
	public function process_validation(){
		...
		if(@fsockopen($_SERVER['REMOTE_ADDR'], $test_port, $errno, $errstr, 5)) {
		    ms(array(
				"status"  => "error",
				"message" => 'Please disable your proxy connection and try again'
			));
		}
	}*/
	...
	public function process(){
		...
		$item = $this->model->get('id, ids,coupan_disc, price', $this->tb_services, ['ids' => $ids, 'status' => 1]);
		if (empty($item)) {
			ms(array(
				"status"  => "error",
		...
		$data = array(
			...
			'payableAmount'      => $payableAmount-$disc,
