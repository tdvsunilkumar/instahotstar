...
require_once(APPPATH.'third_party/unitpay/UnitPay.php'); 
class unitpayment extends MX_Controller {
	public $tb_users;
	...
	public function create_payment($order_details = ""){
		...
		if (!empty($amount) && $amount > 0) {
			$data = (object)array(
				...
				"redirectUrls" => cn("checkout/unitpayment/complete"),
				"cancelUrl"    => cn("checkout/unsuccess"),
			);
			set_session("order_details", $order_details);
			$orderId = 'UNITPAY'.rand(00000,99999);
            $domain = UNITPAY_DOMAIN; // Your working domain: unitpay.money or unitpay.ru
            $secretKey  = UNITPAY_SECRET_KEY;
            $publicId   = UNITPAY_PUBLIC_ID;
            // My item Info
            $itemName = $item->name;
			// My Order Data
			$orderId        = $orderId;
			$orderSum       = $finalPaidAmount;
			$orderDesc      = "Purchase Item";
			$orderCurrency  = 'USD';
			$unitPay = new UnitPay($domain, $secretKey);
			$unitPay
			->setResultUrl($data->redirectUrls)
			->setTestMode()
			->setBackUrl($data->cancelUrl)
			...
			if ($redirectUrl != '') {
				$this->load->view('unitpay/index', ['redirect_url' => $redirectUrl]);
			}else{
				ms(array(
					"status"  => "error",
