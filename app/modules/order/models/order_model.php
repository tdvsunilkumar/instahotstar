...
class order_model extends MY_Model {
	...
	function get_orders_logs_by_search($k){
		...
		if (get_role("user")) {
			...
			if ($k != "" && strlen($k) >= 2) {
				$this->db->where("(`o`.`api_order_id` LIKE '%".$k."%' ESCAPE '!' OR `o`.`id` LIKE '%".$k."%' ESCAPE '!' OR `o`.`status` LIKE '%".$k."%' ESCAPE '!' OR `o`.`link` LIKE '%".$k."%' ESCAPE '!' OR `o`.`visitor_id` LIKE '%".$k."%' ESCAPE '!' OR `o`.`ipadress` LIKE '%".$k."%' ESCAPE '!' OR `o`.`city` LIKE '%".$k."%' ESCAPE '!' OR `o`.`state` LIKE '%".$k."%' ESCAPE '!' OR `o`.`country` LIKE '%".$k."%' ESCAPE '!' OR  `u`.`email` LIKE '%".$k."%' ESCAPE '!' OR  `s`.`name` LIKE '%".$k."%' ESCAPE '!')");
			}
			$this->db->order_by("o.id", 'DESC');
