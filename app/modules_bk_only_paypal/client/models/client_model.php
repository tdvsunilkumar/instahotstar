...
class client_model extends MY_Model {
	...
	function get_client_orders_by($email){
		if ($email) {
			$this->db->select('u.email, o.quantity,o.start_counter,o.remains, o.created,o.charge, o.status, s.name as service_name, s.price, g.transaction_id');
			$this->db->from($this->tb_order." o");
			$this->db->join($this->tb_users." u", "u.id = o.uid", 'left');
			$this->db->join('general_transaction_logs'." g", "g.order_id = o.id", 'left');
			$this->db->join($this->tb_services." s", "s.id = o.service_id", 'left');
