...
class package extends MX_Controller {
	...
	public function index($cate_url_slug = ""){
		...
		if (get_role('admin')) {
			...
			$exists_cate = $this->model->get('id, ids, name,category_content, url_slug, page_title, meta_keywords, meta_description', $this->tb_categories, ['url_slug' => $cate_url_slug , 'status' => 1]);

			$exists_service = $this->model->get('id, ids, cate_id,page_title, meta_keywords, meta_description, package_slug, image,description,name,quantity,price,original_price,min,max', $this->tb_services, ['package_slug' => $cate_url_slug , 'status' => 1]);
