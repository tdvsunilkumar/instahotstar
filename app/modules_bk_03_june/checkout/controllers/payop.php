...
class payop extends MX_Controller {
	...
	public function create_payment($order_details = ""){
		...
		if (!empty($amount) && $amount > 0) {
			...
			$attribures['publicKey'] = PAYOP_PUBLIC_KEY;
        $attribures['order'] = array(
              'id'=>$orderId,
              'amount'=>$finalPaidAmount,
              'currency'=>'USD',
              'description'=>$description,
              'items'=>array(
                  ...
                  'price'=>$finalPaidAmount
              )
              )
        );
        $signature = $this->createSignature(array('id'=>$orderId,
              'amount'=>$finalPaidAmount,
              'currency'=>'USD'));
        //pr($signature);
        $attribures['signature'] = $signature;
        $attribures['payer'] = array(
             'email'=>$email,
             'phone'=>'',
             'name'=>'',
             'extraFields'=>[],
        );
        $attribures['paymentMethod'] = 261;
        //$attribures['customer_postcode'] = 10003;
        $attribures['language'] = "en";
        $attribures['resultUrl'] = $data->redirectUrls;
        $attribures['failPath'] = $data->cancelUrl;
        ...
        CURLOPT_POSTFIELDS => json_encode($attribures),
        ));
